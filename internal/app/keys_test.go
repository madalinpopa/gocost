...
func TestHandleCategoryGroupViewKeys_BackToMonthlyView(t *testing.T) {
	tests := []struct {
		name string
		key  string
	}{
		{"esc should return to monthly view", "esc"},
		{"q should return to monthly view", "q"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			model, cmd := app.handleCategoryGroupViewKeys(tt.key)
			...
			if resultApp.activeView != viewMonthlyOverview {
				t.Errorf("Expected activeView to be viewMonthlyOverview, got %v", resultApp.activeView)
			}
...
func TestHandleCategoryGroupViewKeys_UnknownKey(t *testing.T) {
	app := createTestApp()
	app.activeView = viewCategoryGroup

	model, cmd := app.handleCategoryGroupViewKeys("unknown")
...
func TestHandleCategoryGroupViewKeys_EmptyKey(t *testing.T) {
	app := createTestApp()
	app.activeView = viewCategoryGroup

	model, cmd := app.handleCategoryGroupViewKeys("")
